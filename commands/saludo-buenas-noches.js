...
const refranRandom = Math.floor(Math.random() * refranesbuenasNoches.length);
...
const tituloRandom = Math.floor(Math.random() * titulosbuenasNoches.length);
...
module.exports = {
  ...
  data: new SlashCommandBuilder()
    ...
    .setDescription('Envia un saludo de Buenas noches, todos los dias a las 7 PM'),

  run: async ({ interaction, client, handler }) => {
    const task = schedule.scheduleJob({ hour: 19, minute: 0, tz: 'America/Bogota' }, function () {
      const embed = new EmbedBuilder()
        ...
        .setTimestamp()
        .setThumbnail('https://manicmumlife.files.wordpress.com/2015/04/twitter7pm.png')
