...
const { goodAfternoonTitles } = require('../../embedNeeds/greetingEmbeds/embedTitles');
const { goodAfternoonDescriptions } = require('../../embedNeeds/greetingEmbeds/embedDescription');
const { goodAfternoonImages } = require('../../embedNeeds/greetingEmbeds/embedImages');

module.exports = {
  data: new SlashCommandBuilder()
    .setName('good-afternoon-greeting')
    .setDescription(
      'Sends an embed with a good afternoon greeting, everyday at 12 PM'
    )
  ...
  async execute(interaction) {
    ...
    interaction.reply(`From now on all the good afternoon greetings will be send on ${channel} at the local hour of ${timezone}`);
    ...
    const embedSuccessfulConfiguration = new EmbedBuilder()
      ...
      .setTitle('Good afternoon greetings command has been configured successfully')
      .setDescription(`From now on the ***Good afternoon greetings*** will be send to this channel everyday at 12 PM at the local hour of ${timezone}`)
      .setColor('Random')
      .setTimestamp()
      .setFooter({
